...
if [ $? -eq 0 ]; then
    ...
    if [ -f "/home/azureuser/miniconda3/etc/profile.d/conda.sh" ]; then
...
if [ -d "$HOME/ws/genai/mlopsserver" ]; then
  (
    cd "$HOME/ws/genai/mlopsserver" && . ./env.sh && . ./start.sh 
  )
  cd "$HOME/ws/genai/ml/ed_003"
  . ./setup.sh
fi
...
# --- Shorten the conda virtual environment name in the command prompt ---
...
# Fancy colored single-line prompt
function set_bash_prompt {
  ...
  # Prompt with background colors
  PS1=""

  # Conda env + user@host with green background
  PS1+="\[\e[30;48;5;82m\] ${env}\u@\h \[\e[0m\]"

  # Working directory with blue background
  PS1+="\[\e[30;48;5;33m\] \w \[\e[0m\]"

  # Git branch with purple background (if inside a Git repo)
