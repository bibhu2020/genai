...
if __name__ == "__main__":
    ...
    guid = uuid.uuid4()
    run_name = f"run_logging_artifacts_{guid}"
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment.experiment_id) as run:
        ...
        artifact_path = f"_artifacts/{experiment_name}/"
        artifact_name = "example_artifact.txt"

        with open(artifact_name, "w") as f:
            f.write("This is an example artifact for logging artifacts.")

        mlflow.log_artifact(local_path=artifact_name, artifact_path="text_files")
