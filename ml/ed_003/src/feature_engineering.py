...
# Concrete Strategy for Dropping Columns
# --------------------------------------
# This strategy removes specified columns from a DataFrame.
# Useful for eliminating irrelevant or redundant features 
# during feature engineering.
# It safely drops only columns that exist to avoid errors 
# and logs the dropped columns.

class DropColumns(FeatureEngineeringStrategy):
    def __init__(self, columns_to_drop):
        """
        Initializes the DropColumns strategy with the columns to remove.
    ...
    def apply_transformation(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        existing_cols = [col for col in self.columns_to_drop if col in df_transformed.columns]
        if not existing_cols:
            logging.warning("No specified columns found to drop.")
            return df_transformed

        df_transformed = df_transformed.drop(columns=existing_cols)
        logging.info(f"Columns dropped: {existing_cols}")
        return df_transformed
...
# -------------------------------------
# This strategy converts categorical features into integer labels.
class LabelEncoding(FeatureEngineeringStrategy):
    def __init__(self, features):
        ...
        Parameters:
        features (list): The list of categorical features to label encode.
        """
        self.features = features
        self.encoder = LabelEncoder()  # Dictionary to store LabelEncoder for each feature
